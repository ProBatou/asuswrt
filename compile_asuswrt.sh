...
if [ "$latestVersion" = "$version" ]; then
    ...
    start=$(date +%s)
    find $path/amng-build/ -type f | pv | xargs grep -l -s "X-Frame-Options" | xargs sed -i '/X-Frame-Options/d'
    echo "X-Frame-Options removed"
    find $path/amng-build/ -type f | pv | xargs grep -l -s "x-frame-options" | xargs sed -i '/x-frame-options/d'
    echo "x-frame-options removed"
    find $path/amng-build/ -type f | pv | xargs grep -l -s "x-xss-protection" | xargs sed -i '/x-xss-protection/d'
    echo "x-xss-protection removed"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s "top.location" | xargs sed -i 's/top.location/location/g'
    echo "top.location replaced"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s "parent.location" | xargs sed -i 's/parent.location/location/g'
    echo "parent.location replaced"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s " window.top" | xargs sed -i 's/ window.top/ window/g'
    echo "window.top replaced"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s "parent\." | xargs sed -i 's/parent\.//g'
    echo "parent. removed"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s " top\." | xargs sed -i 's/ top\.//g'
    echo "top. removed"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s $'\ttop\.' | xargs sed -i 's/\ttop\./\t/g'
    echo "  top. removed"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s $'\[top\.' | xargs sed -i 's/\[top\./[/g'
    echo "[top. removed"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s "top.document" | xargs sed -i 's/top.document/document/g'
    echo "top.document replaced"
    find $path/amng-build/release/src/router/www/ -type f | pv | xargs grep -l -s "top.isIE8" | xargs sed -i 's/top.isIE8/isIE8/g'
    ...
    start=$(date +%s)
    make -s -C $path/amng-build/release/src-rt-5.02axhnd.675x/ rt-ax56u
    error=$?
    end=$(date +%s)
    runtime=$((end - start))
